...
export default {
  ...
  methods: {
    ...
    async submitNewTree(approve) {
      ...
      if (approve) {
        submissionObj["is_live_change_ind"] = 2;
        console.log("Approved")
      } else {
        await this.markTreeInactive(this.newTreeToApprove.tree_id)
        submissionObj["is_live_change_ind"] = -3;
        console.log("Denied")
      }
      ...
      <div class="container">
        <div class="box table-container mx-5 my-5">
          <div class="content is-size-5 has-text-centered">Your changes have been successfully updated!
            <br>
            <br>
            This tree has now been added as a live tree and can be viewed
    ...
    <template v-else>
      ...
      <div class="box table-container mx-5 my-5">
        ...
        <div class="buttons is-centered mt-4">
          <button @click="submitNewTree(true)" class="button is-primary">Approve</button>
          <button @click="submitNewTree(false)" class="button is-danger">Deny</button>
        </div>
      </div>
    </template>
  </div>
